...
def clip_loss(img0, img1):
    ...
    for n in range(num_augs):
        loss -= torch.cosine_similarity(
            img0_features[n:n+1], img1_features[n:n+1], dim=1)[0] / num_augs
    return loss
...
def m_clip_text_loss(p, text, num_augs, clip_model):
    loss = 0
    img_augs = []
    with warnings.catch_warnings():
        # RandomPerspective has a really annoying warning
        warnings.simplefilter("ignore")
        for n in range(num_augs):
            img_augs.append(augment_trans_text(p))

    im_batch = torch.cat(img_augs).to(device)
    ...
    with torch.no_grad():
        ...
        image_features[torch.isnan(image_features)] = 0
        text_features[torch.isnan(text_features)] = 0
        image_features[torch.isinf(image_features)] = 0
        text_features[torch.isinf(text_features)] = 0
...
def clip_text_loss(p, text_features, num_augs, text_features_16=None):
    ...
    for n in range(num_augs):
        loss -= torch.cosine_similarity(text_features,
                                        image_features[n:n+1], dim=1)
        if text_features_16 is not None:
            loss -= torch.cosine_similarity(text_features_16,
                                            image_features_16[n:n+1], dim=1)
...
def clip_fc_loss(p, other_img, num_augs, text_features_16=None):
    ...
    for n in range(num_augs):
        loss -= torch.cosine_similarity(
            other_features[n:n+1], p_features[n:n+1], dim=1)
