...
from .parse_color import get_rgb_color
...
def parse_color(s, defs):
    ...
    if isinstance(s, pydiffvg.LinearGradient):
        s = s.stop_colors[0]
    ...
    #     color = torch.tensor([int(rgb[0]) / 255.0, int(rgb[1]) / 255.0, int(rgb[2]) / 255.0, 1.0])
        ...
        rgba = list(get_rgb_color(s))
        if(len(rgba) > 0 and rgba[3] != 0):
...
def parse_common_attrib(node, transform, fill_color, defs):
    ...
    if 'stroke-opacity' in attribs:
        stroke_opacity *= float(attribs['stroke-opacity'])
    if 'opacity' in attribs:
        stroke_opacity *= float(attribs['opacity'])
    if isinstance(stroke_color, torch.Tensor):
        stroke_color[3] = stroke_opacity
    ...
    if 'style' in attribs:
        ...
        if isinstance(fill_color, torch.Tensor):
            ...
            # tensor([0.2549, 0.2745, 0.2863, 1.0000])
            fill_color[3] = fill_opacity
...
def parse_scene(node):
    ...
    if 'width' in node.attrib:
        canvas_width = int(parse_hwv(node.attrib['width']))
    else:
        print('Warning: Can\'t find canvas width.')
    if 'height' in node.attrib:
        canvas_height = int(parse_hwv(node.attrib['height']))
    else:
...
def parse_hwv(string):
    ...
    for unit, coefficient in HWV_UNITS.items():
