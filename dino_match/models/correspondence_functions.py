...
def get_saliency_maps(extractor, image_batch, image_path, curr_num_patches,
    ...
    if low_res_saliency_maps:
        ...
        image_batch, _ = extractor.preprocess(
            image_path, low_res_load_size).to(device)
    saliency_map = extractor.extract_saliency_maps(
        image_batch.to(device)).cpu().numpy()
    ...
    if low_res_saliency_maps:
        ...
        saliency_map_reshaped = reshape_op(
            Image.fromarray(saliency_map_reshaped))
...
def get_saliency_maps_V2(saliency_extractor, image_batch,
    ...
    saliency_map = saliency_extractor.extract_saliency_maps(
        image_batch.to(device), layer=11, facet="attn").cpu().numpy()
    sal_num_patches = saliency_extractor.num_patches

    reshape_op = tf.Resize(num_patches, tf.InterpolationMode.NEAREST)
    saliency_map = np.array(reshape_op(
        Image.fromarray(saliency_map.reshape(sal_num_patches))))
    # saliency_map.shape =  (63, 63)

    # --------------------------------------------
    # print("===========================")
    # upsample = torch.nn.Upsample(size=load_size)
    # ups_sali_map = np.array(upsample(torch.from_numpy(
    #     np.expand_dims(np.expand_dims(saliency_map, axis=0), axis=0))))
    # # ups_sali_map.shape =  (1, 1, 256, 256)

    # ups_sali_map = (ups_sali_map[0, 0, :, :]*255).astype(np.uint8)
    # # fig, ax = plt.subplots(1, 1)
    # fig, ax = plt.subplots()
    # ax.axis('off')
    # ax.imshow(ups_sali_map,
    #           vmin=0, vmax=255, cmap='jet')  # "Blues"

    # savepath = "./" + "ups_sali_map.png"
    # fig.savefig(savepath, bbox_inches='tight', pad_inches=0)
    # --------------------------------------------
    ...
    # --------------------------------------------
    # ups_sali_map = np.array(upsample(torch.from_numpy(
    #     np.expand_dims(np.expand_dims(saliency_map, axis=0), axis=0)).float()))
    # ups_sali_map = (ups_sali_map[0, 0, :, :]*255).astype(np.uint8)
    # _fig, ax = plt.subplots(1, 1)
    # ax.imshow(ups_sali_map)
    # savepath = "./" + "ups_sali_map_thresh.png"
...
def get_saliency_maps_V3(saliency_extractor, image_batch,
                         image, num_patches,
                         load_size, low_res_saliency_maps, device):
    if low_res_saliency_maps and load_size is not None:
        load_size = (load_size[0] // 2, load_size[1] // 2)
        image_batch, _ = saliency_extractor.preprocess_image(image, load_size)
    saliency_map = saliency_extractor.extract_saliency_maps(
...
def map_descriptors_to_clusters(desc, fg_centroids, bg_centroids):
    ...
    D_bg = d_fn(torch.Tensor(desc), torch.Tensor(
        bg_centroids)).max(dim=-1, keepdim=True).values
...
def draw_part_cosegmentation(num_parts: int, segmentation_parts: List[np.ndarray],
    ...
    for parts_seg, pil_image in zip(segmentation_parts, pil_images):
        # np.isin(segmentation, segment_indexes)
        ...
        checkerboard_bg[[x // checkerboard_edge %
                         2 == 0 for x in range(checkerboard_bg.shape[0])], :] = 1
        checkerboard_bg[:, [x // checkerboard_edge % 2 == 1 for x in range(checkerboard_bg.shape[1])]] = \
            1 - checkerboard_bg[:, [x // checkerboard_edge %
                                    2 == 1 for x in range(checkerboard_bg.shape[1])]]
        ...
        fig, ax = plt.subplots()
        ax.axis('off')
        color_list = ["red", "yellow", "blue", "lime",
                      "darkviolet", "magenta", "cyan", "brown", "yellow"]
        cmap = 'jet' if num_parts > 10 else ListedColormap(
            color_list[:num_parts])
        ax.imshow(checkerboard_bg, cmap='gray', vmin=0, vmax=255)
        ax.imshow(masked_image_transparent.astype(np.int32), vmin=0, vmax=255)
        ax.imshow(parts_seg, cmap=cmap, vmin=0, vmax=num_parts -
                  1, alpha=0.5, interpolation='nearest')
        figures.append(fig)
    return figures
...
def draw_correspondences(points1: List[Tuple[float, float]], points2: List[Tuple[float, float]],
    ...
    assert len(points1) == len(
        points2), f"points lengths are incompatible: {len(points1)} != {len(points2)}."
    ...
    for point1, point2, color in zip(points1, points2, colors):
        ...
        circ1_2 = plt.Circle(
            (x1, y1), radius2, facecolor=color, edgecolor='white')
        ...
        circ2_2 = plt.Circle(
            (x2, y2), radius2, facecolor=color, edgecolor='white')
...
def find_correspondences(image_path1: str, image_path2: str, num_pairs: int = 10, load_size: int = 224, layer: int = 9,
    ...
    img1_indices_to_show = torch.arange(
        num_patches1[0] * num_patches1[1], device=device)[indices_to_show]
    ...
    for y1, x1, y2, x2 in zip(img1_y_to_show, img1_x_to_show, img2_y_to_show, img2_x_to_show):
        x1_show = (int(x1) - 1) * \
            extractor.stride[1] + extractor.stride[1] + extractor.p // 2
        y1_show = (int(y1) - 1) * \
            extractor.stride[0] + extractor.stride[0] + extractor.p // 2
        x2_show = (int(x2) - 1) * \
            extractor.stride[1] + extractor.stride[1] + extractor.p // 2
        y2_show = (int(y2) - 1) * \
            extractor.stride[0] + extractor.stride[0] + extractor.p // 2
...
def find_correspondences_v2(image_path1: str, image_path2: str, num_pairs: int = 10, load_size: int = 224, layer: int = 9,
    ...
    if save_dir is not None:
        fig, ax = plt.subplots()
        ax.axis('off')
        ax.imshow(saliency_map1.cpu().detach().numpy().reshape(
            num_patches1), vmin=0, vmax=1, cmap='jet')
        fig.savefig(
            save_dir / f'{Path(image_path1).stem}_saliency_map1.png', bbox_inches='tight', pad_inches=0)
        plt.close(fig)
    # threshold saliency maps to get fg / bg masks
    fg_mask1 = saliency_map1 > thresh
    fg_mask2 = saliency_map2 > thresh
    ...
    if save_dir is not None:
        ...
        ax.imshow(fg_mask1.cpu().detach().numpy().reshape(num_patches1),
                  vmin=0, vmax=1, cmap='jet')
        fig.savefig(
            save_dir / f'{Path(image_path1).stem}_saliency_map1fg.png', bbox_inches='tight', pad_inches=0)
        plt.close(fig)
...
def find_correspondences_v3(image1: Image.Image, image2: Image.Image,
    ...
    # calculate similarity between image1 and image2 descriptors
    similarities = chunk_cosine_sim(
        descriptors1.detach(), descriptors2.detach()).cpu()
...
def find_part_cosegmentation(image_paths: List[str], elbow: float = 0.975,
    ...
    if num_crop_augmentations > 0:
        augmentations_image_paths = augment_images(
            extractor, image_paths, save_dir, num_crop_augmentations, load_size)
    ...
    labels_per_image = np.split(
        labels, np.cumsum(num_descriptors_per_image)[:-1])
    ...
    if save_dir is not None:
        ...
        for path, np, lpi in zip(image_paths, num_patches_list, labels_per_image):
            ...
            if not '_aug_' in stem:
                fig, ax = plt.subplots()
                ax.axis('off')
                ax.imshow(lpi.reshape(np), vmin=0,
    ...
    salient_labels = np.where(votes >= np.ceil(
        num_images * votes_percentage / 100))[0]
    # cluster all parts using k-means:
    # get only foreground descriptors
    fg_masks = [np.isin(labels, salient_labels) for labels in labels_per_image]
    fg_descriptor_list = [desc[:, :, fg_mask[:, 0], :]
                          for fg_mask, desc in zip(fg_masks, descriptors_list)]
    ...
    sampled_fg_descriptors_list = [
        x[:, :, ::sample_interval, :] for x in fg_descriptor_list]
    normalized_all_fg_sampled_descriptors = get_normal_descriptors(
        sampled_fg_descriptors_list)
    ...
    parts_num_descriptors_per_image = [
        np.count_nonzero(mask) for mask in fg_masks]
    part_labels_per_image = np.split(
        part_labels, np.cumsum(parts_num_descriptors_per_image))
    ...
    for image_path, img, num_patches, load_size, descs in zip(image_paths, image_pil_list, num_patches_list, load_size_list, descriptors_list):
        if ('_aug_' in Path(image_path).stem) and not three_stages:
            ...
            continue
        bg_centroids = tuple(i for i in range(
            algorithm.centroids.shape[0]) if not i in salient_labels)

        curr_normalized_descs = descs[0, 0].astype(np.float32)
        faiss.normalize_L2(curr_normalized_descs.copy(
            order='C'))  # in-place operation
        # distance to parts
        ...
        final = CRF_v1(img, dist_to_parts, min_dist_to_bg,
                       num_patches, part_num_labels, load_size)
    ...
    if three_stages:  # if needed, apply third stage
        ...
        if save_dir is not None:
            part_figs = draw_part_cosegmentation(
                part_num_labels, curr_part_segmentations, curr_image_pil_list)
            for image, part_fig in zip(image_paths, part_figs):
                part_fig.savefig(
                    save_dir / f'{Path(image).stem}_vis_sec_stage.png', bbox_inches='tight', pad_inches=0)
            plt.close('all')
        ...
        for part_segment, num_patches in zip(part_segmentations, num_patches_list):
            resized_part_segment = np.array(torch.nn.functional.interpolate(torch.from_numpy(part_segment)
                                                                            ...
                                                                            mode='nearest')[0, 0])
            smoothed_part_labels_per_image.append(
                resized_part_segment.flatten())
        ...
        for image_path, image_labels in zip(image_paths, smoothed_part_labels_per_image):
            if not ('_aug_' in Path(image_path).stem):
                unique_labels = np.unique(
                    image_labels[~np.isnan(image_labels)]).astype(np.int32)
        ...
        for part_segment in smoothed_part_labels_per_image:
            common_parts_masks.append(
                np.isin(part_segment, common_labels).flatten())

        # cluster all final parts using k-means:
        common_descriptor_list = [desc[:, :, mask, :] for mask, desc in zip(
            common_parts_masks, descriptors_list)]
        all_common_descriptors = np.ascontiguousarray(
            np.concatenate(common_descriptor_list, axis=2)[0, 0])
        normalized_all_common_descriptors = all_common_descriptors.astype(
            np.float32)
        # in-place operation
        faiss.normalize_L2(normalized_all_common_descriptors)
        sampled_common_descriptors_list = [
            x[:, :, ::sample_interval, :] for x in common_descriptor_list]
        all_common_sampled_descriptors = np.ascontiguousarray(np.concatenate(sampled_common_descriptors_list,
                                                                             axis=2)[0, 0])
        normalized_all_common_sampled_descriptors = all_common_sampled_descriptors.astype(
            np.float32)
        # in-place operation
        faiss.normalize_L2(normalized_all_common_sampled_descriptors)

        common_part_algorithm = faiss.Kmeans(d=normalized_all_common_sampled_descriptors.shape[1], k=num_parts,
                                             niter=300, nredo=10)
        common_part_algorithm.train(
            normalized_all_common_sampled_descriptors.astype(np.float32))
        ...
        common_part_labels = d.argmax(
            dim=1, keepdim=True).cpu().detach().numpy()  # N,1
        ...
        parts_num_descriptors_per_image = [
            np.count_nonzero(mask) for mask in common_parts_masks]
        common_part_labels_per_image = np.split(
            common_part_labels, np.cumsum(parts_num_descriptors_per_image))
        ...
        for img, num_patches, load_size, descs in zip(image_pil_list, num_patches_list, load_size_list, descriptors_list):
            bg_centroids_1 = tuple(i for i in range(
                algorithm.centroids.shape[0]) if not i in salient_labels)
            bg_centroids_2 = tuple(i for i in range(
                part_algorithm.centroids.shape[0]) if not i in common_labels)
            curr_normalized_descs = descs[0, 0].astype(np.float32)
            ...
            final = CRF_v1(img, dist_to_parts, min_dist_to_bg,
                           num_patches, num_parts, load_size)
